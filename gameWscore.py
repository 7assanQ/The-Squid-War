...
def input(key):       # user input from keyboard
    ...
    global ipaddressInput, userNameInput, inputready, ip_input, playerName, client
    ...
    if key == 'enter':
        ...
        if inputready == 2: # if the name and the ip is ready
            inputready += 1 # to not enter here again
            destroy(info6)
            try: # to try if connected 
                client.connect((ip_input, 9789))
            except: # if not connected
                pass
    ...
    if key == 'r' and inputready == 3: # reset/reload the game
        reloadGame()
...
ip_input = "192.168.2.8" # enter the ip address ipv4
...
info6 = Text('Enter host IP and Nickname',x = -0.32, y = -0.12)
info6.background = True

ipaddressInput = InputField(y = -0.28, limit_content_to = '.1234567890') # to limit the input od IP address to only input numbers
ipaddressInput.tooltip = Tooltip('Enter IP Address\nthen press enter') # when hovering on the text field show 'Enter IP Address'

userNameInput = InputField(y = -0.39) 
