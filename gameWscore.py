...
def sending_messages(c): # to send messages to client
    global score, playerName
    
    text = playerName + ": " + str(score) # to send the player name and the score
    try:  #to try if connected
        c.send(text.encode()) #to send
...
def gameOver():         # display a gameover text with the score. 
    ...
    # need to add some condition if there is no host ignore sending a message so the game doesn't crash     
    sending_messages(client)
...
def input(key):       # user input from keyboard
    ...
    global ipaddressInput, portInput, userNameInput, inputready, ip_input, port, playerName, client

    gunSound += random.randint(1, 2)   # to get a random number between 1, 2

    if key == 'enter':
        if ipaddressInput.text != '':
            ip_input = ipaddressInput.text # set the global Ip adress to the text field
            destroy(ipaddressInput) # remove the text field     
            ipaddressInput.text = '' # set it back to ''
            inputready += 1   # if 2 to allow the game to start
        if portInput.text != '':
            port = int(portInput.text) # set the global Ip adress to the text field
            destroy(portInput) # remove the text field     
            portInput.text = '' # set it back to ''
            inputready += 1   # if 2 to allow the game to start    

        if userNameInput.text != '':  
            playerName = userNameInput.text  # set the global player name adress to the text field
            destroy(userNameInput) 
            userNameInput.text = '' # set it back to ''
            inputready += 1   # if 2 to allow the game to start

        if inputready == 3: # if the name and the ip is ready
            inputready += 1 # to not enter here again
            destroy(info6)
            try: # to try if connected 
    ...
    if key == 'r' and inputready == 4: # reset/reload the game
        reloadGame()
...
score = 0   
level = '\nLevel: 1'
inputready = 0 # if 0 do not allow the game to start
ip_input = "" # enter the ip address ipv4
port = 0
playerName = '' # for the player name
...
info3 = Text() # for the starting options priented after the name and ip is given
destroy(info3)

info5 = Text('The Squid War', origin = (0,-8))
...
info6 = Text('Enter host IP, PORT and Nickname',x = -0.4, y = -0.08)
info6.background = True

ipaddressInput = InputField(y = -0.22, limit_content_to = '.1234567890') # to limit the input od IP address to only input numbers
ipaddressInput.tooltip = Tooltip('Enter IP Address\nthen press enter') # when hovering on the text field show 'Enter IP Address'

portInput = InputField(y = -0.33, limit_content_to = '1234567890') # to limit the input od IP address to only input numbers
portInput.tooltip = Tooltip('Enter PORT\nthen press enter') # when hovering on the text field show 'Enter IP Address'

userNameInput = InputField(y = -0.44) 
userNameInput.tooltip = Tooltip('Enter a Nickname\nthen press enter') 
